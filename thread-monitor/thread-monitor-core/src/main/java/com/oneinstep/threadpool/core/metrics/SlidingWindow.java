...
public abstract class SlidingWindow<T> {
    ...
    protected SlidingWindow(int paneCount, long intervalInMs) {
        if (paneCount <= 0) {
            throw new IllegalArgumentException("pane count is invalid: " + paneCount);
        }
        if (intervalInMs <= 0) {
            throw new IllegalArgumentException("total time interval of the sliding window should be positive");
        }
        if (intervalInMs % paneCount != 0) {
            throw new IllegalArgumentException("total time interval needs to be evenly divided");
        }
